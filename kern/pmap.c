...
init_memory(void) {
    ...
    assert(!res);
    if (res)
        panic("init_memory: cannot map kernel!");
    ...
    res = map_physical_region(&kspace, (uintptr_t) __text_start, PADDR(__text_end), __text_end - __text_start, PROT_R | PROT_W);

    assert(!res);
    if (res)
        panic("init_memory: cannnot ");
