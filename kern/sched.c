...
sched_yield(void) {
    ...
    #if 1
    int cur = (curenv - envs) % sizeof(*curenv);
    int it = (cur + 1) & NENV;

    for (; it != cur && envs[it].env_status != ENV_RUNNABLE; it = (it + 1) & NENV)
        ;

    if (envs[it].env_status == ENV_RUNNABLE || envs[it].env_status == ENV_RUNNING)
        env_run(&envs[it]);
    #else
    env_run(&envs[0]);
    #endif
